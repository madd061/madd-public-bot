...
module.exports = async (message) => {
  ...
  if (siradakilevel < data.messageXP) {
    ...
    const levelUpChannel = message.guild.channels.cache.get('1310795025340170323'); // Buraya level up bildirimlerini göndermek istediğiniz kanalın ID'sini girin
    if (levelUpChannel) {
      levelUpChannel.send({ content: `> **[** ${message.author} **]** kullanıcısı level atladı TEBRİKLER!`, files: [{ attachment: levelUp, name: `levelup-${message.member.id}.png` }] });
    }
    return;
  }

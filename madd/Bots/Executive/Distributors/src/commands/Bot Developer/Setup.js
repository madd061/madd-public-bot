...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    message.reply({ embeds: [ancient],components: [row] })
    }
...
!kur staff <örnek: @yetkilialıimdm >
...
Staffs Roles: (${ayar.staffs.length > 0 ? `${ayar.staffs.map(x => `<@&${x}>`).join(",")}` : "\`YOK\`"})
Staff Roles: (${ayar.staff.length > 0 ? `${ayar.staff.map(x => `<@&${x}>`).join(",")}` : "\`YOK\`"})
...
Underworld Role: (${ayar.underworldRole.length > 0 ? `${ayar.underworldRole.map(x => `<@&${x}>`).join(",")}` : "\`YOK\`"})
...
Underworld Log Channel: (${ayar.banLogChannel.length ? `<#${ayar.banLogChannel}>` : "\`YOK\`"})
...
{ name: ["staff","staf"], conf: "staff", cmdName: "Yetkili AlımRol(leri)" },
...
{ name: ["underworld","doomRole","underworldRole","underworldRole","underworldRoles"], conf: "underworldRole", cmdName: "Underworld Rol" },

...
const ancientcim = require("../../../../../../config.json")
const { red, Hello } = require("../../../../src/configs/emojis.json")
...
module.exports = {
  ...
  run: async (client, message, args) => {
    if(!message.channel.name.includes("bot-commands")) return message.reply(`Bu Komutu Sadece ${message.guild.channels.cache.filter(x => x.name.includes("bot-commands")).map(x => x).join(", ")} kanallarında kullanabilirsiniz.`).then(x => {
        message.react(red)
...
if (emoti <= 44 && emoti >= 23) {
    ...
    const path = require('path'); // path modülünü require etmeyi unutmuşsunuz, bu satırı eklemelisiniz.
    ...
    ctx.beginPath();
    ctx.globalAlpha = 0.5; // globalAlpha'nın sonuna noktalı virgül eklenmeli

  ...
  run: async (client, message, args, embed, prefix) => {
    ...
    if (!isim && !yaş) {
      ...
      message.reply({ embeds: [new EmbedBuilder()
        .setThumbnail()
        .setAuthor({ name: message.guild.name, iconURL: message.guild.iconURL({ dynamic: true }) })
        .setDescription(`${red} Kayıt edilecek kişinin ismini ve yaşını girin.`)
      ] }).then((e) => setTimeout(() => { e.delete(); }, 5000));
      return;
    }

    const yasKilit = allah.kayıt?.yasKilit;
    const minYas = allah.kayıt?.minYas || 0;
    
    if (yasKilit && yaş && parseInt(yaş) < minYas) {
      message.react(red);
      message.reply({ embeds: [new EmbedBuilder()
        .setThumbnail()
        .setAuthor({ name: message.guild.name, iconURL: message.guild.iconURL({ dynamic: true }) })
        .setDescription(`${red} ${minYas} yaş altı kullanıcıların kayıt işlemi yapılamaz.`)
      ] }).then((e) => setTimeout(() => { e.delete(); }, 5000));
      return;
    }
    ...
    if (!yaş) {
      setName = `${uye.user.username.includes(ayar.ikinciTag) ? ayar.ikinciTag : (ayar.ikinciTag ? ayar.ikinciTag : (ayar.ikinciTag || ""))} ${isim}`;
    } else {
      setName = `${uye.user.username.includes(ayar.ikinciTag) ? ayar.ikinciTag : (ayar.ikinciTag ? ayar.ikinciTag : (ayar.ikinciTag || ""))} ${isim} | ${yaş}`;
    }
    ...
    if (ayar.erkekRolleri.some(x => uye.roles.cache.has(x)) || ayar.kizRolleri.some(y => uye.roles.cache.has(y))) {
      ...
      message.reply({ embeds: [new EmbedBuilder()
        .setThumbnail()
        .setDescription(`${red} Bu üye zaten kayıtlı durumda, yanlış kayıt ettiyseniz kayıtsız atarak tekrar kayıt edebilirsiniz.`)
      ] }).then((e) => setTimeout(() => { e.delete(); }, 5000));
      return;
    }
...
let msg = await message.channel.send({ embeds: [ancient], components : [ row4, row],})
 ...
 collector.on("collect", async (menu) => {
  if (menu.customId === "kayit") {
    if (menu.values[0] === "ancient-kayit") {
...
if(button.customId === "MAN") {
  ...
  .setDescription(`${uye.toString()} **ERKEK** olarak kayıt edildi! ${green} `)
    ...
    if(ayar.chatChannel && client.channels.cache.has(ayar.chatChannel)) client.channels.cache.get(ayar.chatChannel).send({ content:`> ${Erkek} Aramıza ${uye} Yakışıklısı katıldı onu Merhaba ile karşılayın.`}).then((e) => setTimeout(() => { e.delete(); }, 15000));
           ...
           $set: {
              ...
              if (uye && uye.voice && uye.voice.channel && ayar.registerParents.includes(uye.voice.channel.parentId)) {
                setTimeout(() => {
                 ...
                 uye.send({ content: `${uye.toString()} kayıtınız tamamlandığı için teyit kanallarından **Public Ses Odamıza** tarafımca çekildiniz. - İyi sohbetlerr dilerimm \` ${message.guild.name} \` `}).catch(() => {});
                }, 1000);
              }
...
if(button.customId === "WOMAN") {
  ...
  if (msg) msg.delete();
  button.reply({ embeds: [ancient], components: [row3], ephemeral: false });
    ...
    if(ayar.chatChannel && client.channels.cache.has(ayar.chatChannel)) client.channels.cache.get(ayar.chatChannel).send({ content:`> ${Kadin} Aramıza ${uye} Güzelliği katıldı onu Merhaba ile karşılayın.`}).then((e) => setTimeout(() => { e.delete(); }, 15000));

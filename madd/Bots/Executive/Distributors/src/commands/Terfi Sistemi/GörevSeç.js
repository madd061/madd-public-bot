...
const { red, Tac, Hello, welcome2, ok, nokta } = require("../../../../src/configs/emojis.json")
...
module.exports = {
 ...
 let buttons = new Discord.ActionRowBuilder().addComponents(
  ...
  let buttonn = new Discord.ActionRowBuilder().addComponents(
    ...
    return }
    if(taskk.filter((x) => x.active).length >= 1) return message.reply({embeds:[embed.setDescription(`En Fazla 1 Aktif Görevin Bulunabilir, Yeni Görev Almak İçin Eski Görevlerini Bitirmelisin!`)]});
    let member = message.member;
    let mesaj = await message.reply({ components: [buttons, buttonn], embeds: [embed.setDescription(`
    ${Hello} **Selamlar ${member} Aşağıda Bulunan Menüden İstediğin 1 Görevi Alabilirsin!**

    ${nokta} Invite Görevini Alırsan 5 İla 10 Invite Arası 10 Saatli Süreli Invite Görevi Verir
    
    ${nokta} Ses Görevini Alırsan 60 İla 300 Dakika Arası 10 Saatli Süreli Ses Görevi Verir
    
    ${nokta} Mesaj Görevini Alırsan 300 İla 400 Mesaj Arası 10 Saatli Süreli Mesaj Görevi Verir
    
    ${nokta} Taglı Görevini Alırsan 1 İla 5 Arası 10 Saatli Süreli Taglı Görevi Verir
    
    ${nokta} Yetkili Görevini Alırsan 1 İla 3 Arası 10 Saatli Süreli Yetkili Görevi Verir
    
    ${nokta} Kayıt Görevini Alırsan 5 İla 20 Arası 10 Saatli Süreli Kayıt Görevi Verir
    
    ${nokta} Görev Ödüllerini 35 Coin Olarak Verir Ve Artırıla Bilir

...
module.exports = {
    ...
    if(!uye) return message.channel.send("\`.sikayet <@ancient/ID> <chat-sesli-taciz-dm> <Şikayet Açıklaması>\`")
    ...
    let tür = args[1];
    if(tür !== "chat" && tür !== "sesli" && tür !== "taciz" && tür !== "dm") return message.channel.send("\`.sikayet <@ancient/ID> <chat-sesli-taciz-dm> <Şikayet Açıklaması>\`")
    let şikayetbilgi = args.slice(2).join(' ');
    if(!şikayetbilgi) return message.channel.send("\`.sikayet <@ancient/ID> <chat-sesli-taciz-dm> <Şikayet Açıklaması>\`")
    message.react(green);
    message.channel.send(`Şikayetiniz yetkililere başarıyla iletilmiştir en kısa zamanda sonuçlandırılacaktır.`).then(x => {
        ...
        if (client.channels.cache.find(c => c.name === "sikayet_log")) {
            client.channels.cache.find(c => c.name === "sikayet_log").send({ embeds: [sikiminkafasi] });
        }
    });
}
}
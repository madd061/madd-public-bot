...
let küfürler = ["owospu çocu","owowpsu çocuğu","Oc","oc","oç","Oç","allahoc","allahoç","allahamk","allahaq","0r0spuc0cu","4n4n1 sk3r1m","p1c","@n@nı skrm","evladi","orsb","orsbcogu","amnskm","anaskm","oc","abaza","abazan","ag",
...
"zikik","zikim","ziksiiin","ziksiin","zulliyetini","zviyetini","sg"];
...
module.exports = async (message) => {
    ...
    if (allah.Guard.kufurEngel && küfürler.some(word => new RegExp("(\\b)+(" + word + ")+(\\b)", "gui").test(message.content))) return sendChat(message, "Güzel kardeşim küfür senin o güzel diline hiç yakışıyormu ayıp.")

    if(message.content && message.content.length && message.content.length >= 165)  return sendChat(message, "Lütfen uzun mesaj göndermemeye özen gösteriniz.")
    const Caps = (message.content.match(/[A-ZĞÇÖIÜ]/gm) || []).length;
    if ((allah.Guard.capsEngel) && (Caps / message.content.length) >= 0.9) return sendChat(message, "Caps-Lock kullanmayı lütfen azaltalım.")
...
client.on("messageCreate", async (message) => {
    const LIMIT = 9; // Mesaj sınırı
    const DIFF = 2000; // Zaman farkı, milisaniye cinsinden (örneğin 2 saniye)
    const TIME = 60000; // Kullanıcı veri saklama süresi, 1 dakika
    const TIMEOUT_DURATION = 30 * 1000; // 30 saniye zaman aşımı süresi
  ...
  if (usersMap.has(message.author.id)) {
    ...
    if (difference > DIFF) {
      ...
      msgCount++;
      if (msgCount === LIMIT) { // 9 mesajdan sonra timeout
        sonMesajlar(message, 30);
        usersMap.delete(message.author.id);

        // Kullanıcıya 30 saniye timeout uygulama
        await message.member.timeout(TIMEOUT_DURATION, "Metin Kanallarında Flood Yapmak!").catch(err => {
          console.error("Zaman aşımı uygulanamadı:", err);
        });

        await message.reply({ content: `Sohbet kanallarını kirletme sebebiyle \`30 saniye\` süresince zaman aşımına alındınız, mesajlar temizlendi. Lütfen yavaşlayın. ${message.member}` })
          .then(x => setTimeout(() => {
            x.delete().catch(err => { });
          }, 7500))
          .catch(err => { });
...
client.on('messageUpdate', async (oldMessage, newMessage) => {
    if(newMessage.webhookID || newMessage.author.id || newMessage.channel.type === ChannelType.DM) return;
    if (await client.checkPermission(client, oldMessage.author.id, "full") || await client.checkPermission(client, oldMessage.author.id, "chatguard")) return;
    if (newMessage.channel.id == ["photo-chat"] && newMessage.attachments.size < 1) await newMessage.delete();
    ...
    async function sendChat(message, content) {
        if (getLimit.get(message.author.id) == 3) {
            ...
            await message.reply({content: `${message.member} Sohbet kanallarında ki kurallara uymadığın için \` 10 Dakika \` susturuldun.`}).then(x => setTimeout(() => {
                x.delete().catch(err => {})
            }, 7500)).catch(err => {})
            const duration = "10m" ? ms("10m") : undefined;
            return client.penalize(message.guild.id, client.user.id, "CHAT-MUTE", true, message.author.id, "Metin Kanallarında Flood Yapmak!", true, Date.now() + duration);
        } else {

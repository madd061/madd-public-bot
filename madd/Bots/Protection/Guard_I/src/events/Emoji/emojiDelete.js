...
.setDescription(`
${entry.executor} üyesi izinsiz emoji sildi ve kullanıcıyı karantinaya alıp emojiyi yeniden yükledim \`${moment(Date.now()).format("LLL")}\`

> Yetkili (${entry.executor} - \`${entry.executor.id}\`)
> Emoji \`${emoji.name}\` - \`${emoji.id}\``)

let ancientGuardLog = await emoji.guild.channels.cache.find(x => x.name == "protection_log").send({ embeds: [ancient], components: [row] });

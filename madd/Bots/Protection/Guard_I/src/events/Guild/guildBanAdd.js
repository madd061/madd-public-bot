...
let ancientGuardLog = await member.guild.channels.cache.find(x => x.name == "protection_log").send({ embeds: [ancient], components: [row] });
...
return member.guild.channels.cache.find(x => x.name == "protection_log").send({ embeds: [ancient] });
} else {
    BanLimit[entry.executor.id].Now += 1;
    setTimeout(() => {
        BanLimit[entry.executor.id] = {

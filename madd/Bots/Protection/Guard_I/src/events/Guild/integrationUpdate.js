...
.setDescription(`
> ${entry.executor} üyesi izinsiz entegrasyon güncelledi ve güncellediği gibi cezalandırıldı \`${moment(Date.now()).format("LLL")}\`

> Yetkili: (${entry.executor} - \`${entry.executor.id}\`)`)

let ancientGuardLog = await guil.guild.channels.cache.find(x => x.name == "protection_log").send({ embeds: [ancient], components: [row] });
